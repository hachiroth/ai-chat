  ...
  <aside class="sticky top-0 h-screen bg-base-200 flex flex-col justify-between transition-all duration-300"
    ...
    <div class="flex flex-col py-3 px-4">
      ...
      <div v-for="item in asideMens" :key="item.icon" class="flex items-center overflow-hidden cursor-default"
        :class="{ 'hover:bg-neutral/10 transition-colors duration-300 rounded cursor-pointer': !isCollapse }" @click="$router.push(item.path || '')">
    ...
    <ul class="menu grow scroll-area overflow-x-hidden mr-1 whitespace-nowrap transition-opacity duration-300"
      :class="{ 'opacity-0': isCollapse }">
    ...
    <RouterLink to="/" class="aside-footer p-2 mb-6 mx-3 flex items-center rounded transition-all duration-300" :class="[isCollapse ? 'hover:[&_.avatar]:scale-125' : 'hover:bg-neutral/10']">
      <div v-if="userStore.isLoggedIn" class="avatar avatar-placeholder transition-all duration-300">
  ...
  <main class="grow flex flex-col">
    <div class="p-4 flex gap-1 justify-between items-center font-bold sticky top-0 z-50 bg-base-100">
      ...
      <div v-if="!userStore.isLoggedIn" class="text-sm flex gap-2 items-center">
        <span>
          您正在使用<span class="text-bg-primary p-1 rounded">匿名对话</span>
        </span>
